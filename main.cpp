...
int main() {
  ...
  if (!renderer.init_sdl("Thanksgiving Party", 1280, 720)) return 0;
  ...
  while (!menu.wants_to_quit) {
    ...
    if (lastFPSDisplay + 200 < lastTime) {
      ...
      if (menu.current == None) {
        ...
        if (damage != 0) player.hurt_sound = true;
      }
    }

    if (menu.current ==
        None)  // no menu is displayed, the player is currently playing
    {
      ...
      map.speed =
          menu.difficulty == 0 ? 0.8 : (menu.difficulty == 1 ? 1.2 : 2.5);

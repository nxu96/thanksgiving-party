...
Map::Map()
  ...
  std::cout << "Level loaded with " << sprites.size() << " sprites"
            << std::endl;
...
Door Map::get_door(ushort x, ushort y) {
  ...
  for (i = 0; i < doors.size() - 1; i++) {
    if (doors.at(i).x == x && doors.at(i).y == y) break;
  }
  return doors.at(i);
}
...
bool Map::update_doors(float player_x, float player_y, float dt) {
  for (unsigned int i = doors.size(); i > 0; i--) {
    float sqr_dist = pow(player_x - doors.at(i - 1).x, 2) +
                     pow(player_y - doors.at(i - 1).y, 2);
    if (sqr_dist < 10) {
...
void Map::sort_sprites(float player_x, float player_y) {
  ...
  for (unsigned int i = sprites.size() - 1; i > 0; i--) {
    sprites.at(i).sqr_dist =
        pow(player_x - sprites.at(i).x, 2) + pow(player_y - sprites.at(i).y, 2);

    if (sprites.at(i).type == Temporary &&
        sprites.at(i).start_time + 500 < SDL_GetTicks())
      delete_sprite(i);
  }
...
std::vector<Sprite> const& Map::get_sprites() { return sprites; }

void Map::delete_sprite(ushort id) { sprites.erase(sprites.begin() + id); }
...
void Map::update_sprites(float player_x, float player_y, float dt) {
  ...
  if (dist[px + py * w] != 0) update_dist_map(px, py);
...
Uint32 Map::get_pixel(SDL_Surface* source, ushort x, ushort y) {
  ...
  Uint8* p = (Uint8*)source->pixels + y * source->pitch +
